...
study_target <- Sys.getenv('AHRI_TRE_STUDY', 'The Biology of Subclinical Asymptomic TB')
study_names <- if (is.list(studies) && is.list(studies$studies)) {
  vapply(studies$studies, function(e) if (!is.null(e$study$name)) as.character(e$study$name[[1]]) else NA_character_, character(1), USE.NAMES = FALSE)
...
if (is.list(datasets) && is.list(datasets$datasets)) {
  dataset_names <- vapply(datasets$datasets, function(e) if (!is.null(e$catalog$asset$name)) as.character(e$catalog$asset$name[[1]]) else NA_character_, character(1), USE.NAMES = FALSE)
