...
target_study_name <- "IMPACT BP Training"
...
candidate_entity_names <- character(0)
...
if (!is.null(study)) {
  ...
  if (nrow(study_assets) > 0 && "name" %in% names(study_assets)) {
    candidate_entity_names <- unique(c(candidate_entity_names, as.character(study_assets$name)))
  }

  domain_variables <- get_domain_variables(datastore, domain)
  domain_vocabularies <- get_vocabularies(datastore, domain)

  if (nrow(study_assets) > 0) {
  ...
  cat("[INFO] Study not found; continuing with residual metadata cleanup for known IMPACT objects.\n")
}
