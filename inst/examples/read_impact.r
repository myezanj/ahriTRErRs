#!/usr/bin/env Rscript
#
# Read datasets from the "IMPACT BP Training" study.
# This is a diagnostic script to list studies and datasets.
#
# Environment variables:
#   AHRI_TRE_STUDY          - Study name (default: IMPACT BP Training)
...
study_names <- if (is.list(studies) && is.list(studies$studies)) {
  vapply(studies$studies, function(e) if (!is.null(e$study$name)) as.character(e$study$name[[1]]) else NA_character_,
         character(1), USE.NAMES = FALSE)
} else if (is.data.frame(studies) && "name" %in% names(studies)) {
  as.character(studies$name)
} else {
  character(0)
}
...
cat(sprintf("[INFO] Using study: %s\n", study_name))
...
if (is.list(datasets) && is.list(datasets$datasets)) {
  dataset_names <- vapply(datasets$datasets, function(e) if (!is.null(e$catalog$asset$name)) as.character(e$catalog$asset$name[[1]]) else NA_character_,
                          character(1), USE.NAMES = FALSE)
...
cat(sprintf("[INFO] Dataset entries found: %d\n", length(dataset_names)))
for (i in seq_len(min(10L, length(dataset_names)))) {
  cat(sprintf("[INFO] Dataset %d: %s\n", i, dataset_names[[i]]))
}