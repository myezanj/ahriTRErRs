...
tre_cli_args_from_body <- function(kind, body) { tokens <- strsplit(kind, ".", fixed=TRUE)[[1]]; args <- as.list(tokens); if (length(body)==0) return(unlist(args)); for (key in names(body)) { value <- body[[key]]; if (is.null(value)) next; cli_key <- gsub("_", "-", tre_normalize_body_key(key)); flag <- paste0("--", cli_key); if (is.logical(value) && length(value)==1) { if (isTRUE(value)) args <- c(args, flag); next }; if (length(value)==0 || is.list(value)) next; for (item in as.character(value)) args <- c(args, flag, item) }; unlist(args) }
