...
# Install required packages
  ...
  miss <- pkgs[!vapply(pkgs, requireNamespace, quietly = TRUE, logical(1))]
  if (length(miss)) install.packages(miss, repos = CRAN)
  invisible(sapply(pkgs, library, character.only = TRUE))
}
ensure(c("jsonlite", "roxygen2", "devtools", "remotes"))
...
generate_wrappers <- function(schema = "docs/tre_schema_map.json") {
  ...
  parse_imp <- function(inp) {
    ...
    flat <- gsub("\\?", " ", flat)
    params <- list()
    m <- gregexpr("--[a-zA-Z0-9-]+", flat)
    if (m[[1]][1] != -1) {
      for (f in regmatches(flat, m)[[1]]) {
        key <- substr(f, 3, nchar(f))
        n <- norm(key)
        if (!is.null(n)) params[[n]] <- list(param = n, key = key)
      }
    }
    m <- gregexpr("(?<![a-zA-Z0-9\\-])-[a-zA-Z][a-zA-Z0-9-]+", flat, perl = TRUE)
    if (m[[1]][1] != -1) {
      for (s in regmatches(flat, m)[[1]]) {
        key <- substr(s, 2, nchar(s))
        n <- norm(key)
        if (!is.null(n)) params[[n]] <- list(param = n, key = key)
      }
    }
    stop_words <- c("optional","repeated","none","or","and","with","without","run")
    for (seg in strsplit(flat, "[,;]")[[1]]) {
      seg <- trimws(seg)
      if (seg == "") next
      tok <- seg
      if (grepl("^[A-Za-z][A-Za-z0-9-]*$", seg)) tok <- tolower(seg)
      else if (grepl("^[A-Za-z][A-Za-z0-9-]*\\s+name$", seg)) tok <- paste0(tolower(gsub("\\s+name$", "", seg)), "_name")
      else if (grepl("^[A-Za-z][A-Za-z0-9-]*\\s+id$", seg)) tok <- paste0(tolower(gsub("\\s+id$", "", seg)), "_id")
      if (tok %in% stop_words) next
      n <- norm(tok)
      if (!is.null(n)) {
        key <- gsub("_", "-", tok)
        params[[n]] <- list(param = n, key = key)
      }
    }
    res <- list(); seen <- c()
    for (p in params) {
  ...
  for (cat in names(json$categories)) {
    for (item in json$categories[[cat]]) {
  ...
  for (cat in names(cat_map)) {
    ...
    for (row in cat_rows) {
      ...
      lines <- c(lines, sig)
      fld <- c()
      for (p in uniq) fld <- c(fld, paste0("    \"", p$key, "\" = ", p$param))
...
# ----------------------------------------------------------------------------
# Generate wrappers, build documentation, clean src
# ----------------------------------------------------------------------------
generate_wrappers()
roxygen2::roxygenise(".")
...
unlink(file.path(release_dir, paste0(pkg, "_*.tar.gz")))
cat("[INFO] Building source tarball...\n")
system(paste0("R CMD build . --no-build-vignettes --no-manual --compact-vignettes=gs+qpdf"))
tarball <- list.files(".", pattern = paste0("^", pkg, "_.*\\.tar\\.gz$"), full.names = TRUE)
if (length(tarball)) {
  file.copy(tarball, release_dir, overwrite = TRUE)
  file.remove(tarball)
  cat("[INFO] Created", file.path(release_dir, basename(tarball)), "\n")
}

# ----------------------------------------------------------------------------
# Optionally install the built package
# ----------------------------------------------------------------------------
if (Sys.getenv("AHRI_TRE_AUTO_INSTALL_RELEASE", "true") != "false") {
  install_target <- file.path(release_dir, paste0(pkg, "_", ver, ".tar.gz"))
  if (file.exists(install_target)) {
    cat("[INFO] Installing main package...\n")
    install.packages(install_target, repos = NULL, type = "source")
  }
}
cat("[INFO] Done.\n")